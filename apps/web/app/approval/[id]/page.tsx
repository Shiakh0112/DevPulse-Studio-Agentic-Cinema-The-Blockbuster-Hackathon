  ...
  if (error || !incident) {
    return (
      <div className="p-8 bg-rose-950/40 border border-rose-800/60 rounded-xl text-rose-200">
        ...
        <Link href="/feed" className="inline-flex items-center gap-2 text-sm text-text-secondary hover:text-white mt-4 underline">
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-16">
      ...
      <div className="flex items-center justify-between border-b border-border-subtle pb-4">
        <div className="flex items-center gap-4">
          <Link
            ...
            className="p-2 bg-bg-card hover:bg-slate-700 text-text-secondary rounded-lg transition-colors"
          >
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono text-xs px-3 py-1 rounded-full bg-bg-card text-accent-DEFAULT border border-border-subtle">
      ...
      {approvedSuccess && (
        <div className="p-4 bg-emerald-950/80 border border-emerald-600 rounded-xl text-emerald-200 flex items-center justify-between shadow-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-text-primary">Human Approval Recorded</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Rationale Card */}
        <div className="md:col-span-2 bg-bg-card/80 border border-border-subtle/60 rounded-xl p-6 shadow-xl space-y-4">
          ...
          {finalProposal?.file_path && (
            <div className="text-xs font-mono text-text-secondary bg-bg-main px-3 py-2 rounded-lg border border-border-subtle flex items-center gap-2">
              <span className="text-text-secondary">Target File:</span>
              <span className="text-text-primary">{finalProposal.file_path}</span>
            </div>
          )}
        </div>

        {/* Confidence & Risk Card */}
        <div className="bg-bg-card/80 border border-border-subtle/60 rounded-xl p-6 shadow-xl flex flex-col justify-between space-y-4">
          <h3 className="text-lg font-bold text-text-primary border-b border-border-subtle/60 pb-3">
            Risk & AI Confidence
          </h3>
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full bg-bg-main h-2 rounded-full overflow-hidden border border-border-subtle">
                <div
              ...
              <span
                className={`text-xs font-bold px-3 py-1 rounded-full border ${
                  finalProposal?.risk === "HIGH"
                    ...
                    ? "bg-accent-DEFAULT/20 text-accent-DEFAULT border-accent-DEFAULT/40"
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border-subtle/60">
              <span className="text-xs text-text-secondary">Governance Status</span>
              <span
                className={`text-xs font-bold px-3 py-1 rounded-full border ${
                  isApproved
                    ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/40"
                    : isRejected
                    ? "bg-rose-500/20 text-rose-400 border-rose-500/40"
                    : "bg-accent-DEFAULT/20 text-accent-DEFAULT border-accent-DEFAULT/40 animate-pulse"
                }`}
              >
                {decisionState}
              </span>
      ...
      <div className="space-y-3">
        <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
          <GitPullRequest className="w-5 h-5 text-accent-DEFAULT" />
        ...
        {finalProposal ? (
          ...
          <div className="p-6 bg-bg-main rounded-xl border border-border-subtle text-center text-text-secondary font-mono">
      ...
      <div className="bg-bg-card/80 border border-border-subtle/60 rounded-xl p-6 shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-border-subtle/60 pb-3">
          <h3 className="text-lg font-bold text-text-primary flex items-center gap-2">
        ...
        {verificationRuns.length === 0 ? (
          <div className="text-sm text-text-secondary py-4 text-center font-mono">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {verificationRuns.map((run) => (
              <div
                key={run.run_id}
                className="bg-bg-main border border-border-subtle rounded-lg p-3 flex items-center justify-between"
              >
                <div className="flex items-center gap-3">
                  {run.passed ? (
                  ...
                  <div>
                    <div className="text-sm font-semibold text-text-primary">{run.test_name}</div>
                  </div>
                </div>
      ...
      {/* Action Footer */}
      <div className="bg-bg-main border border-border-subtle rounded-xl p-6 flex flex-wrap items-center justify-between gap-4 shadow-xl">
        <div className="space-y-1">
          <div className="text-sm font-semibold text-text-primary flex items-center gap-2">
            <span>Governance Action Bar</span>
            <span className="text-xs px-2.5 py-0.5 rounded bg-bg-card text-text-secondary border border-border-subtle font-mono">
              Current Role: {role}
            </span>
          </div>
        ...
        <div>
          {isRoleAuthorized && isNeedsReview ? (
            ...
            <button
              disabled
              className="px-6 py-3 bg-bg-card text-text-secondary font-bold text-sm rounded-xl cursor-not-allowed border border-border-subtle/60 flex items-center gap-2"
            >
