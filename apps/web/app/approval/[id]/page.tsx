  ...
  if (error || !incident) {
    return (
      <div className="p-8 bg-rose-950/40 border border-rose-800/60 rounded-xl text-rose-200">
        ...
        <Link href="/feed" className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white mt-4 underline">
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-16">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono text-xs px-3 py-1 rounded-full bg-slate-800 text-amber-400 border border-slate-700">
      ...
      {approvedSuccess && (
        <div className="p-4 bg-emerald-950/80 border border-emerald-600 rounded-xl text-emerald-200 flex items-center justify-between shadow-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-slate-100">Human Approval Recorded</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Rationale Card */}
        <div className="md:col-span-2 bg-slate-800/80 border border-slate-700/60 rounded-xl p-6 shadow-xl space-y-4">
          ...
          {finalProposal?.file_path && (
            <div className="text-xs font-mono text-slate-400 bg-slate-900 px-3 py-2 rounded-lg border border-slate-800 flex items-center gap-2">
              <span className="text-slate-500">Target File:</span>
              <span className="text-slate-200">{finalProposal.file_path}</span>
            </div>
          )}
        </div>

        {/* Confidence & Risk Card */}
        <div className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-6 shadow-xl flex flex-col justify-between space-y-4">
          <h3 className="text-lg font-bold text-slate-100 border-b border-slate-700/60 pb-3">
            Risk & AI Confidence
          </h3>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-700/60">
              <span className="text-xs text-slate-400">Governance Status</span>
              <span
                className={`text-xs font-bold px-3 py-1 rounded-full border ${
                  isApproved
                    ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/40"
      ...
      <div className="space-y-3">
        ...
        {finalProposal ? (
          ...
          <div className="p-6 bg-slate-900 rounded-xl border border-slate-800 text-center text-slate-400 font-mono">
      ...
      <div className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-6 shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-700/60 pb-3">
          <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
        ...
        {verificationRuns.length === 0 ? (
          <div className="text-sm text-slate-400 py-4 text-center font-mono">
            ...
            {verificationRuns.map((run) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-slate-200">{run.test_name}</div>
                  </div>
                </div>
      ...
      {/* Action Footer */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-wrap items-center justify-between gap-4 shadow-xl">
        ...
        <div>
          {isRoleAuthorized && isNeedsReview ? (
            ...
            <button
              disabled
              className="px-6 py-3 bg-slate-800 text-slate-500 font-bold text-sm rounded-xl cursor-not-allowed border border-slate-700/60 flex items-center gap-2"
            >
