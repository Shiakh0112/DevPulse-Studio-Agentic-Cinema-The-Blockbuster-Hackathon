    ...
    <div className="min-h-screen bg-[#0B0E14] text-[#FFFFFF] p-6 md:p-10">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#1F293D] pb-6">
          ...
          <div className="flex items-center space-x-3 text-xs text-[#8A94A6] bg-[#121824]/80 px-4 py-2 rounded-lg border border-[#1F293D]">
            <span>Auto-Refreshing (5s)</span>
            <button
              onClick={fetchIncidents}
              className="text-[#6366F1] hover:text-[#6366F1] font-semibold underline ml-2 flex items-center gap-1"
            >
        ...
        <div className="bg-[#121824]/60 backdrop-blur border border-[#1F293D]/60 rounded-xl p-4 sm:p-5 space-y-4">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl border text-xs font-semibold transition-all cursor-pointer shadow-md ${
                mobileFiltersOpen || activeFiltersCount > 0
                  ? "bg-[#0B0E14] border-[#6366F1]/80 text-[#6366F1] ring-2 ring-amber-500/20 shadow-amber-500/10"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {activeFiltersCount > 0 && (
                  <span className="px-2 py-0.5 text-[10px] rounded-full bg-[#6366F1] text-slate-950 font-bold font-mono">
                    {activeFiltersCount} ACTIVE
                  </span>
                )}
              </div>
              <ChevronDown
                className={`w-4 h-4 text-[#6366F1] transition-transform duration-200 ${
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold font-mono transition-all cursor-pointer ${
                showEmergenciesFirst
                  ...
                  : "bg-[#0B0E14] text-[#8A94A6] border border-[#1F293D] hover:text-[#FFFFFF]"
              }`}
            >
              <AlertOctagon className={`w-4 h-4 ${showEmergenciesFirst ? "text-rose-400 animate-pulse" : "text-[#8A94A6]"}`} />
              <span>Show Emergencies First</span>
              <span className={`px-1.5 py-0.5 text-[10px] rounded ${showEmergenciesFirst ? "bg-rose-900 text-white" : "bg-[#121824] text-[#8A94A6]"}`}>
        ...
        {loading && incidents.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-[#8A94A6] space-y-4">
          ...
          <div className="bg-[#0B0E14] border border-[#1F293D] rounded-2xl p-12 text-center max-w-2xl mx-auto space-y-5 shadow-2xl">
            <div className="w-16 h-16 bg-[#121824]/80 border border-[#1F293D] text-[#6366F1] rounded-2xl mx-auto flex items-center justify-center shadow-lg">
            ...
            {/* Code Snippet Box */}
            <div className="bg-[#0B0E14] border border-[#1F293D] rounded-xl p-4 text-left space-y-2 shadow-inner">
              <div className="flex items-center gap-2 text-xs font-mono text-[#8A94A6] border-b border-[#1F293D] pb-2">
                <Terminal className="w-4 h-4 text-[#6366F1]" />
                <span>Command to trigger synthetic crash:</span>
              </div>
              <pre className="font-mono text-xs text-[#6366F1] overflow-x-auto whitespace-pre pt-1">
