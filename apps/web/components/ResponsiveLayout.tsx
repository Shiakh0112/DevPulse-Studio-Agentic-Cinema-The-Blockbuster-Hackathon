...
export function ResponsiveLayout({ children }: ResponsiveLayoutProps) {
  ...
  const navLinks = [
    { href: "/feed", label: "Live Incident Feed", icon: <Activity className="w-4 h-4 text-amber-400" /> },
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen w-full bg-slate-900 text-slate-100">
      ...
      <header className="md:hidden sticky top-0 z-40 bg-slate-950/95 backdrop-blur-lg border-b border-slate-800 px-4 py-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => setMobileMenuOpen(true)}
            className="p-2 text-slate-300 hover:text-white rounded-lg bg-slate-900 border border-slate-800 cursor-pointer transition-colors"
            aria-label="Open Navigation Sidebar Drawer"
          >
            <Menu className="w-5 h-5 text-amber-400" />
          </button>
          <Link href="/feed" className="flex items-center gap-2">
            <div className="p-1.5 bg-amber-500/10 rounded-md border border-amber-500/30 text-amber-500">
              <Zap className="w-4 h-4 fill-amber-500/20" />
            </div>
            <span className="font-bold text-base tracking-tight text-slate-100">
              DevPulse <span className="text-amber-500">Studio</span>
            </span>
          </Link>
        </div>
      ...
      >
        ...
        <div
          className={`relative w-72 max-w-[80vw] bg-slate-950 border-r border-slate-800 h-full p-5 shadow-2xl flex flex-col justify-between z-10 transition-transform duration-300 ease-in-out transform ${
          ...
          <div>
            ...
            <div className="flex items-center justify-between pb-4 mb-6 border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-amber-500/10 rounded-lg border border-amber-500/30 text-amber-500">
                  <Zap className="w-5 h-5 fill-amber-500/20" />
                </div>
                <div>
                  <h2 className="font-bold text-base tracking-tight text-slate-100 leading-none">
                    DevPulse <span className="text-amber-500">Studio</span>
                  </h2>
                  <span className="text-[9px] uppercase font-mono text-slate-400">AIOps Engine</span>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setMobileMenuOpen(false)}
                className="p-1.5 text-slate-400 hover:text-white rounded-lg bg-slate-900 border border-slate-800 cursor-pointer transition-colors"
      ...
      {/* Desktop Sidebar (>= md) */}
      <aside className="hidden md:flex w-64 bg-slate-950 border-r border-slate-800 flex-col justify-between p-4 shrink-0 h-screen sticky top-0">
        <div>
          <div className="flex items-center gap-3 px-3 py-4 mb-6 border-b border-slate-800">
            ...
            <div>
              <h1 className="font-bold text-lg tracking-tight text-slate-100 leading-none">
                DevPulse <span className="text-amber-500 font-extrabold">Studio</span>
              </h1>
              <span className="text-[10px] uppercase font-mono text-slate-400 tracking-wider">AIOps Engine</span>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="hidden md:flex h-16 border-b border-slate-800 bg-slate-950/50 backdrop-blur px-6 items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-2 text-xs font-mono text-slate-400">
            <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-300">ENV: Production</span>
            <span>/</span>
            <span className="text-amber-400">Media Pipeline Watcher</span>
          </div>
